            ...
            for log in op.logs.iter() {
                ...
                hm.insert(
                    ...
                    Log {
                        ...
                        base_url,
                        state: log_state,
                        start_inclusive: log.temporal_interval.clone().map(|k| k.start_inclusive),
                        end_exclusive: log.temporal_interval.clone().map(|k| k.end_exclusive),
...
fn test() {
    ...
    assert!(nb_logs > 0);
    assert_eq!(
        ll.find_by_id(&base64::decode("zxFW7tUufK/zh1vZaS6b6RpxZ0qwF+ysAdJbd87MOwg=").unwrap())
            .unwrap()
            .base_url,
